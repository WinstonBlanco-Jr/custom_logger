...
def get_campaign_ids(session: requests.Session, branch_id: str, headers: dict, token: str) -> list[dict]:
    url = f'https://at-vc-gtw.deliveryhero.io/api/v1/entities/TB_AE/vendors/{branch_id}/campaigns'
    headers.update({
...
def get_campaign_data(session: requests.Session, branch_id: str, headers: dict, token: str, campaign_id: str) -> list[
    dict]:
    url = 'https://at-vc-gtw.deliveryhero.io/api/v1/entities/TB_AE/reporting'
    headers.update({
...
def uae_main():
    ...
    output = []
    with create_session('winston@kitopi.com', 'Wemovefast@123', headers) as s:
    ...
    df.to_csv('./data/uae_talabat.csv', index=False)
